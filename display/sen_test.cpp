...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods) {
	...
	case GLFW_KEY_I:
		std::cin >> pct;
		glUniform1f(glGetUniformLocation(shader->mShader, "pct"), pct);
		std::cout << "pct: " << pct << std::endl;
		nbFrames = 31;
		std::cout << "Reset nbFrames!" << std::endl;
		break;
	case GLFW_KEY_W:
		baseColor += 8;
	...
	std::cout << "current diff" << baseColor * pct << std::endl;
}
int main()
{

	std::string prefix = "freq_";//"fremw2_";
	srand(time(NULL));
	...
	glUniform1i(glGetUniformLocation(adjustShader.mShader, "baseColor"), 128);
	glUniform1f(glGetUniformLocation(adjustShader.mShader, "pct"), 0.3);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if (testing)
			nbFrames++;
		if (nbFrames > 31 + 62 * 100 + 32 && testing) testing = false;
