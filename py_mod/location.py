...
localizer = Localizer()
while True:
    # sample two registers
    ret = localizer.update((sample_pixel, sample_avg))
    if ret is not None:
        print(ret)
'''
class Localizer:
    def __init__(self, mouse_frame_rate=1600):
        self.MOUSE_FRAME_RATE = mouse_frame_rate
        self.frames = deque(maxlen=self.MOUSE_FRAME_RATE * 2)
        self.frames_accum = deque(maxlen=self.MOUSE_FRAME_RATE * 2)
        self.len_e = 3000
        self.last_succ = None
        self.last_ts = None
