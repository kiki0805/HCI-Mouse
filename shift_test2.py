...
if __name__ == '__main__':
    dirs = os.listdir('D:\\kiki\\Documents\\Nutstore\\shift_test') + os.listdir('D:\\kiki\\Documents\\Nutstore\\shift_test\\data2')
    valid_dirs = []
    for i in dirs:
        if i[:4] == 'test':
            valid_dirs.append(i)
    ...
    existed = []
    f = open('shift_record_bkp')
    lines = f.readlines()
    pat = re.compile('test_freq\d*_\d+_\d.csv')
    for l in lines:
        result = pat.findall(l)
        if result != []:
            existed.append(result[0])
    # print(existed)
    for i in existed:
        valid_dirs.remove(i)

    existed = []
    f = open('shift_record_p1_bkp')
    lines = f.readlines()
    pat = re.compile('test_freq\d*_\d+_\d.csv')
    for l in lines:
        result = pat.findall(l)
