...
#     print(BW.shape)
    ...
    hspace, angles, dists = hough_line_peaks(H, T, R, min_distance=0, min_angle=0, num_peaks=20)
#     hspace, angles, dists = hough_line_peaks(H, T, R, num_peaks=30)
    ...
    for i in range(top):
        for j in range(len(hspace)):
            ...
            if abs(a1 - a2) < 2 or abs(abs(a1 - a2) - 90) < 2:
...
for i in imgs_val:
    ...
    # r_im = ImageEnhance.Contrast(r_im).enhance(1)
    w_im = Image.open(white_img_name(i))
    # w_im = ImageEnhance.Contrast(w_im).enhance(1)
    r_ang, r_line, _ = calc_angle(r_im)
    w_ang, w_line, bim = calc_angle(w_im)
    # ax.imshow(w_line)
    # ax2.imshow(bim)
    # plt.show()
    # plt.pause(3)
